...
<header class="header_main"><div class="header_menu"><a class="tukibetu" href="{{url('/tarukame_totalling',)}}" >月別データ</a>
    ...
    <a class="nyuryoku" href="{{url('/register_member',['user_name'=>$UserName])}}" >入力画面</a><a class="nenbetu" href="{{url('/tarukame_nenbetu',)}}" >年別データ</a><div class="user_name">こんにちは&nbsp&nbsp{{$UserName}}さん</div></div>
    ...
    @if($recodes==false)
         ...
         <tr>
          <div class="Master_0"> <td>データが登録されてません</td></div>
           ...
           $karenda="karenda_zero_tukibetu";
           $kensaku="kensaku_zero_tukibetu";
...
<div class={{$henkou}}><canvas id="myPieChart1" width="1200" height="350" class="mypie2"></canvas> </div>
<table class="table table-striped table_nenbetu">
   <div class="itiran"><th>件数</th> <th>内容</th> <th>支出</th><th>収入</th><th>登録日時</th><th>編集</th> </div>
  ...
  <div class="Balance_column"> </div>
   <div class="sisyutu_goukei">支出合計</div><div class={{$total_sisyutu_name}}>{{number_format($sisyutgoukei->total_spending)}}円</div>
